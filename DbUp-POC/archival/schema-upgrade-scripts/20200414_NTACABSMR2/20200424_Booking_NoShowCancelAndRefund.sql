...
/****** Object:  StoredProcedure [dbo].[Booking_NoShowCancelAndRefund]    Script Date: 4/24/2020 12:55:42 PM ******/
SET ANSI_NULLS ON
GO
...
--
-- =============================================
ALTER PROCEDURE [dbo].[Booking_NoShowCancelAndRefund]
        ...
        IF @RefundReasonId = 4 --- Conditional Offer Expiration
            BEGIN
                IF @LicenceStateId = 10 --Conditional Offer
					...
					-- We do not want to add cancelation letters in NoShow Case
					-- We want only the COL Expiry Letter to be generated if needed 
					IF(@ManualCancelConditionalOffer is not null and @ManualCancelConditionalOffer = 1)
							BEGIN
								  ...
								  -- Update CO Expiry Date
									...
									-- we dont want the standard cancelation letter to be sent 
						...
						if(@LetterRequestTypeId = 3) -- we want to sent only COL- expiry letter 
							BEGIN	
