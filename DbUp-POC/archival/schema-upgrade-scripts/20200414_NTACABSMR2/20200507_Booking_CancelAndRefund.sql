...
@HistoryChangeId AS INT;
    ...
    BEGIN TRY
		...
		IF @RefundReasonId IN (6) OR (@RefundReasonId IN (1,2) AND @IsExpiredLicence = 5 )  --If Licence has been expired then updated open-fail booking to 'Completed -Fail(Closed)
			BEGIN			
			...
			BEGIN
				UPDATE  dbo.Booking
					SET BookingStatusId             = 5,
						...
						ModifiedDate                = @ModifiedDate
				WHERE   BookingId IN (SELECT BookingId
									  FROM   @BookingIds) and BookingStatusid in (1, 2);
			END
