...
/****** Object:  StoredProcedure [dbo].[LicenceVL_Search]    Script Date: 25.06.2024 11:11:09 ******/
SET ANSI_NULLS ON
GO
...
(
	...
	and @LicenceStateMaster is null
)
BEGIN
...
SET @LicenceNumber = REPLACE(@LicenceNumber, '''', '')
...
SET @LicenceStateMaster = REPLACE(@LicenceStateMaster, '''', '')
...
SET @SearchSql = @SearchSql + ' AND H.HolderName LIKE ''%' + @HolderName + '%'''
	...
	IF 	@Ccsn IS NOT NULL
	BEGIN
SET @SearchSql = @SearchSql + ' AND H.Ccsn LIKE ''%' + @Ccsn + '%'''
	END	
...
IF 	@LicenceNumber IS NOT NULL
	BEGIN
SET @SearchSql = @SearchSql + ' AND L.LicenceNumber LIKE ''%' + @LicenceNumber + '%'''
	END	
...
SET @SearchSql = @SearchSql + ' AND H.DateOfBirth = ''' + convert(VARCHAR, @DateOfBirth) + ''''
	...
	IF 	@LicenceStateMaster IS NOT NULL
	BEGIN
SET @SearchSql = @SearchSql + ' AND SM.LicenceStateMaster = ''' + convert(VARCHAR, @LicenceStateMaster) + ''''
	END
...
@LicenceStateMaster = @LicenceStateMaster
