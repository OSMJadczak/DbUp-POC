...
/****** Object:  StoredProcedure [dbo].[USP_DWH_GetVehicleAgeBreakdownByCounty]    Script Date: 10/20/2020 1:50:23 PM ******/
SET ANSI_NULLS ON
GO
...
=============================================
*/
ALTER PROCEDURE [dbo].[USP_DWH_GetVehicleAgeBreakdownByCounty]
				@Status NVarChar(20)
...
Union All
Select 2 As VehicleRank, '5 to 6 years' As Age, dbo.Fn_DWH_GetAlphaNumeric(Replace(Replace(County, '.', ''), 'CO ', '')) As County, Count(*) As NumberOfVehicles From #Vehicles Where VehicleAge >= 5 And VehicleAge <= 6 Group By County
Union All
Select 3 As VehicleRank, '7 to 8 years' As Age, dbo.Fn_DWH_GetAlphaNumeric(Replace(Replace(County, '.', ''), 'CO ', '')) As County, Count(*) As NumberOfVehicles From #Vehicles Where VehicleAge >= 7 And VehicleAge <= 8 Group By County
Union All
Select 4 As VehicleRank, '9 to 10 years' As Age, dbo.Fn_DWH_GetAlphaNumeric(Replace(Replace(County, '.', ''), 'CO ', '')) As County, Count(*) As NumberOfVehicles From #Vehicles Where VehicleAge >= 9 And VehicleAge <= 10 Group By County
Union All
Select 5 As VehicleRank, '11 + years' As Age, dbo.Fn_DWH_GetAlphaNumeric(Replace(Replace(County, '.', ''), 'CO ', '')) As County, Count(*) As NumberOfVehicles From #Vehicles Where VehicleAge >= 11 Group By County
