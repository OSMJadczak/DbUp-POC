...
/****** Object:  StoredProcedure [dbo].[USP_DWH_GetVehicleAgeBreakdown]    Script Date: 5/14/2020 4:04:34 PM ******/
SET ANSI_NULLS ON
GO
...
Exec USP_DWH_GetVehicleAgeBreakdown 'ACTIVE'

=============================================
*/
ALTER PROCEDURE [dbo].[USP_DWH_GetVehicleAgeBreakdown] 
				@Status NVarChar(20)
				--, @VehicleAge SmallInt
As
	...
	, Case IsNumeric(Left(VM.Reg_Nbr, 2))
		When 1 Then 
			Case 
				...
				When Cast(Left(VM.Reg_Nbr, 2) As SmallInt) > 20 Then Year(GetDate()) - (1900 + Cast(Left(VM.Reg_Nbr, 2) As SmallInt))
...
Select Distinct VehicleAge, Count(*) As NumberOfVehicles From #Vehicles Group By VehicleAge Order By VehicleAge 
