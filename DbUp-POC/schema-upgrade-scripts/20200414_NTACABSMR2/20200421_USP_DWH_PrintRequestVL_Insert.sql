...
/****** Object:  StoredProcedure [dbo].[USP_DWH_PrintRequestVL_Insert]    Script Date: 5/13/2020 11:15:21 AM ******/
SET ANSI_NULLS ON
GO
...
Declare @PrintRequestMasterId Int,
    ...
    begin
            SELECT   @InspectionDate = BookingDateTime,
                    ...
                    @TestCentreAddress = 
                    CASE COALESCE(PostCode, '') WHEN '' 
                    THEN AddressLine1 + coalesce(', ' + AddressLine2, '') + coalesce(', ' + AddressLine3, '') + coalesce(', ' + Town, '') + coalesce(', CO.' + CountyName, '')
                    ELSE AddressLine1 + coalesce(', ' + AddressLine2, '') + coalesce(', ' + AddressLine3, '') + coalesce(', ' + Town, '') + coalesce(' ' + PostCode, '') 
                    END,
    ...
    BEGIN
        Begin Transaction CreatePrintRequest
            ...
            (
                ...
                [LicenceIssueDate],
                [LicenceExpiryDate],
                [LicenceLastReactivationDate], --Expiry Date + 1 years
                ...
                DATEADD(year, 1, dbo.StripTime(LicenceExpiryDate)),
			...
			if (@LetterRequestTypeId is not null and @LetterRequestTypeId in (2,3,5,15,8,25,61,62))		                                 
