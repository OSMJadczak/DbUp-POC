...
$servernames = get-content C:\HealthCheckMonitoringPowershellScript\Server.txt
...
$path = "C:\HealthCheckMonitoringPowershellScript\"
...
set @query2008r2_cpu = 'SELECT cpu_count AS Logical_CPU_Count, hyperthread_ratio AS Hyperthread_Ratio,cpu_count/hyperthread_ratio AS Physical_CPU_Count,physical_memory_in_bytes/1024/1024 AS Physical_Memory_in_MB FROM sys.dm_os_sys_info'
...
#2. Top 10 Memory Consuming Objects
...
ConvertTo-HTML -head $a -PostContent $frag1,$frag2,$frag3,$frag4,$frag5,$frag6,$frag7,$frag8 -PreContent "<h1>SQL Server Health Check Report</h1>" | Out-File $OutputFile_new
...
Send-MailMessage -to "aindira@openskydata.com","aindira@openskydata.com" -from "no_replyuat2@nationaltransport.ie" -SmtpServer "NTA-CABUT2-APP2" -subject $subject -credential $anonCredentials -BodyAsHtml "$body" -Attachments $attachment #$OutputFile_new
