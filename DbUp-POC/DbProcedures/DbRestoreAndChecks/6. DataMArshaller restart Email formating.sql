USE [DataMarshaller]
GO
/****** Object:  StoredProcedure [marshaller].[sch_RestartDataMarshallerJob]    Script Date: 3/25/2020 11:11:32 AM ******/
SET ANSI_NULLS ON
GO
...
-- Description:	Job checks if Data marshaller is running on time and restart after 1 hour
--
--
-- =============================================
ALTER PROCEDURE [marshaller].[sch_RestartDataMarshallerJob] 
...
BEGIN
	...
	IF (DATEDIFF(s,@lastRun,@getDate)>=3600)
begin 
...
set @mailbody += 'Restarting Data Marshaller at   ';
set @mailbody += cast(GETDATE() as nvarchar(40));
set @mailbody += '<br/> <br/> <br/> <br/>';


set @mailbody += 'Sincerly, <br/> Your Humble DataMarshallerRestart Job <br/> ';


exec [marshaller].[RestartDataMarshaller];
...
--send email
 ...
 @profile_name                  = 'CABS',        
  -- @recipients                  = 'rpodgorski@openskydata.com;anosal@openskydata.com',
   ...
   @subject                   = 'Data Marshaller RESTART - PREPRODUCTION',
   @body                       = @mailbody, 
   @body_format                = 'HTML', 
   @importance                  = 'NORMAL',
   @sensitivity                 = 'NORMAL',
